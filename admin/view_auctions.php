...
if($_SERVER["REQUEST_METHOD"]=="POST"){
    ...
    //move uploaded file
    move_uploaded_file($temp_file1,"./includes/images/auctions/$upload_file");

    //insert query
    $insert_query = "insert into auctions(upload_file, file_name) values(?,?)";
    ...
    if($result_query){
        echo "<script>alert('Inserted successfully.'); window.location.href = 'view_auctions.php';</script>";
        exit();
    }else{
...
<div class="orders-container" id="table-container1">
    ...
    <table id="datatable dark" class="orders-table">
        ...
        <tbody>
            <?php
                $select_query = "select * from auctions";
                ...
                if($result_query){
                    ...
                    while($row = mysqli_fetch_assoc($result_query)){
                        ...
                        <td>
                            <a href="./includes/images/auctions/' . $file_name.'" target="_blank">
                        ...
                        <td>
                            <button class="btn btn-primary" style="border-radius: 5px; padding: 7px 15px; background-color: blue; width: fit-content">
                                <a href="update_auctions.php?update_id=' . $id . '" class="text-light" style="text-decoration: none; color: white;">Edit</a>
                            </button>
                        </td>
                        <td>
...
<script type="text/javascript">
    function chkalert(id){
        ...
        if(sts){
            document.location.href = `view_auctions.php?delete_id=${id}`;
        }
    }
</script>
...
if (isset($_GET['delete_id'])) {
    ...
    $select_query = "SELECT file_name FROM auctions WHERE id=$id";
    $result_query = mysqli_query($con, $select_query);
    ...
    if ($result_query) {
        ...
        $file_path1 = './includes/images/auctions/' . $file_name;
        ...
        if ($result_query) {
            ...
            echo "<script>alert('Deleted successfully'); window.location.href = 'view_auctions.php';</script>";
            exit();
        } else {
            echo "<script>alert('Error deleting record'); window.location.href = 'view_auctions.php';</script>";
        }
    } else {
        echo "<script>alert('Error fetching images'); window.location.href = 'view_auctions.php';</script>";
    }
}
?>
